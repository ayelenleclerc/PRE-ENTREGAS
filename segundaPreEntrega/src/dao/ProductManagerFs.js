...
class ProductManagerFs {
  ...
  async addProduct(
    ...
    let id =
      this.getProducts().then((resultado) => resultado.length) == undefined
        ? 1
        : this.getProducts().then((resultado) => resultado.length + 1);
    ...
    let ListaCode = listaProduct().then((resultado) =>
      resultado.map((elemento) => elemento.code)
    );
    let mismoCode = ListaCode.then((resultado) =>
      resultado.includes(producto1.code)
    );
    if (elementoVacio || elementoUnd) {
      ...
      await fs.promises.writeFile(
        this.path,
        JSON.stringify([...listaProductos, producto1])
      );
      return true;
    }
  }
  ...
  async getProductById(id) {
    ...
    const productoBuscado = this.getProducts().then((resultado) =>
      resultado.find((element) => element.id == id)
    );
  ...
  async updateProduct(
    ...
    if ((await Product) != false) {
      ...
      if (elementoVacio || elementoUnd) {
        ...
        let listaModificada = listaProduct().then((resultado) =>
          resultado.map((elemento) => {
            if (id == elemento.id) {
              elemento = producto1;
            }
        ...
        await fs.promises.writeFile(
          this.path,
          JSON.stringify(await listaModificada)
        );
      }
    } else {
